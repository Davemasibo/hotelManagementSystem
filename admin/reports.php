<div class="container-fluid py-3">
  ...
  <div class="page-header mb-4">
    <h5 class="page-title"><i class="fa fa-chart-bar mr-2"></i>Reports &amp; Analytics</h5>
    <div class="d-flex align-items-center flex-wrap" style="gap:6px">
      <input type="date" id="rep-from" class="form-control form-control-sm" style="width:140px" value="<?php echo date('Y-m-01'); ?>">
      <span class="text-muted">to</span>
      <input type="date" id="rep-to" class="form-control form-control-sm" style="width:140px" value="<?php echo date('Y-m-d'); ?>">
  ...
  <div class="row mb-4" id="kpi-row">
    <div class="col-6 col-md-3 mb-3">
      <div class="card kpi-card kpi-blue shadow-sm">
    ...
    <div class="col-6 col-md-3 mb-3">
      <div class="card kpi-card kpi-green shadow-sm">
        <div class="kpi-accent"></div>
        <div class="card-body text-center">
    ...
    <div class="col-6 col-md-3 mb-3">
      <div class="card kpi-card kpi-gold shadow-sm">
        <div class="kpi-accent"></div>
        <div class="card-body text-center">
          <div class="kpi-label">ADR</div>
          <div class="kpi-value" id="kpi-adr" data-toggle="tooltip" title="Average Daily Rate = Revenue / Room Nights Sold">—</div>
        </div>
      </div>
    </div>
    <div class="col-6 col-md-3 mb-3">
      <div class="card kpi-card kpi-amber shadow-sm">
        ...
        <div class="card-body text-center">
          ...
          <div class="kpi-value" id="kpi-revpar" data-toggle="tooltip" title="Revenue Per Available Room = Revenue / Total Available Room-Nights">—</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <!-- Detailed Table -->
  <div class="card">
    <div class="card-header"><strong>Revenue by Category &mdash; Detail</strong></div>
    <div class="card-body p-0">
      <table class="table table-sm mb-0" id="cat-table">
        <thead>
          <tr><th>Category</th><th>Revenue</th><th>% Share</th></tr>
        </thead>
        <tbody id="cat-tbody">
          <tr><td colspan="3" class="text-center text-muted py-3">Run a report to see data</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
// Apply global dark defaults for Chart.js
Chart.defaults.color = 'rgba(232,228,220,0.6)';
      ...
      datasets: [
        {
          label: 'Revenue (KES)',
          data: [],
          backgroundColor: 'rgba(196,145,58,0.7)',
          yAxisID: 'y'
        },
        {
          label: 'Check-Ins',
...
function runReport() {
  ...
  $.getJSON('ajax.php?action=get_reports&from=' + from + '&to=' + to, function(d) {
    ...
    $('#kpi-rev').text('KES ' + parseFloat(d.revenue).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
    $('#kpi-adr').text('KES ' + parseFloat(d.adr).toFixed(2));
    $('#kpi-revpar').text('KES ' + parseFloat(d.revpar).toFixed(2));
    ...
    (d.by_category || []).forEach(function(c) {
      ...
      rows += '<tr><td>' + c.name + '</td><td>KES ' + parseFloat(c.revenue).toFixed(2) + '</td><td>' + pct + '%</td></tr>';
    });
