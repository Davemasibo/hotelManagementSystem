...
.badge-issued    { background:#ffc107; color:#000; }
.badge-partial   { background:#17a2b8; color:#fff; }
.badge-paid      { background:#28a745; color:#fff; }
.badge-cancelled { background:#dc3545; color:#fff; }
.badge-refunded  { background:#6f42c1; color:#fff; }
</style>

<div class="container-fluid py-3">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0"><i class="fa fa-file-invoice-dollar mr-2 text-success"></i>Billing & Invoices</h5>
    <div class="d-flex align-items-center">
      <select id="status-filter" class="custom-select custom-select-sm mr-2" style="width:auto">
  ...
  <div class="row mb-3">
    <div class="col-6 col-md-3 mb-2">
      <div class="card border-success"><div class="card-body py-2 text-center">
        <div class="text-success font-weight-bold" style="font-size:1.3rem">$<?php echo number_format($rev_today,2); ?></div>
        <small class="text-muted">Today's Revenue</small>
      </div></div>
    </div>
    <div class="col-6 col-md-3 mb-2">
      <div class="card border-primary"><div class="card-body py-2 text-center">
        <div class="text-primary font-weight-bold" style="font-size:1.3rem">$<?php echo number_format($rev_month,2); ?></div>
        <small class="text-muted">This Month</small>
      </div></div>
    </div>
    <div class="col-6 col-md-3 mb-2">
      <div class="card border-warning"><div class="card-body py-2 text-center">
        <div class="text-warning font-weight-bold" style="font-size:1.3rem"><?php echo $unpaid['c']; ?></div>
        <small class="text-muted">Unpaid Invoices</small>
      </div></div>
    </div>
    <div class="col-6 col-md-3 mb-2">
      <div class="card border-danger"><div class="card-body py-2 text-center">
        <div class="text-danger font-weight-bold" style="font-size:1.3rem">$<?php echo number_format($unpaid['b'],2); ?></div>
    ...
    <div class="card-body p-0">
      <table class="table table-hover mb-0" id="billing-table">
        <thead class="thead-dark">
          <tr><th>#</th><th>Invoice No</th><th>Guest / Ref</th><th>Room</th><th>Period</th><th>Total</th><th>Paid</th><th>Balance</th><th>Status</th><th>Actions</th></tr>
        </thead>
        <tbody>
        <?php
        $invs = $conn->query(
        ...
        <tr>
          ...
          <td>
            <?php if ($inv['is_vip']): ?><span class="badge" style="background:#ffc107;color:#000;font-size:.65rem">VIP</span> <?php endif; ?>
            <?php echo htmlspecialchars($gname); ?>
            <br><small class="text-muted"><?php echo htmlspecialchars($inv['ref_no']); ?></small>
          </td>
          <td><?php echo htmlspecialchars($inv['room']??'—'); ?><br><small class="text-muted"><?php echo htmlspecialchars($inv['category']??''); ?></small></td>
          <td><small><?php echo substr($inv['date_in']??'',0,10); ?> → <?php echo substr($inv['date_out']??'',0,10); ?></small></td>
          <td class="font-weight-bold">$<?php echo number_format($inv['total'],2); ?></td>
          <td class="text-success">$<?php echo number_format($inv['amount_paid'],2); ?></td>
          <td class="<?php echo $inv['balance']>0?'text-danger':'text-success'; ?>">
